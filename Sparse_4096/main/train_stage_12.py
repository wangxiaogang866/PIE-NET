...
parser.add_argument('--gpu', type=int, default=1, help='GPU to use [default: GPU 0]')    # change gpu device number
...
#        total_task_6_loss = 0.0
        ...
        for j in range(num_batch):
            ...
            #batch_regression_direction = np.zeros((BATCH_SIZE,NUM_POINT,3),np.float32)  # each point normal estimation
...
#                                 ops['task_4_acc'],ops['task_5_loss'],ops['task_6_loss'],ops['loss']],feed_dict=feed_dict)
            summary, step, _, task_1_loss_val, task_1_recall_val, task_1_acc_val, \
                                 task_1_1_loss_val, task_1_1_recall_val, task_1_1_acc_val, loss_val = sess.run([ops['merged'], ops['step'], \
