...
for i = 1:num_ele_one
    ...
    %2,
    all_pre_label_ind = unique([all_ele;open_pre_label_ind]);
    all_points = input_pl(all_pre_label_ind,:);        
    num_points = size(all_points,1);
        
    num_neibor = 5;
    Weight_matrix = Distance_Points1_Points2_matrix(all_points, all_points);        
    [~,idx_wei_mat_1] = sort(Weight_matrix,2);
    idx_wei_mat = idx_wei_mat_1(:,2:num_neibor);
    Weight_matrix_mask = zeros(num_points,num_points);
    for row_wei_mat = 1:num_points
        temp_mask = zeros(1,num_points);
    ...
    source_local_idx = find(all_pre_label_ind == ele_one(i)); 
    all_ele_del = setdiff(all_ele,All_pair_idx_mat(idx_row1,:));
...
function local_idx_neibor = find_path_neibor(source_local_idx,target_local_idx,G)

    [TR,D] = shortestpathtree(G,source_local_idx,target_local_idx,'OutputForm','cell');
    path_length = cellfun(@(x) size(x,2), TR, 'Unif',0);
    path_length = cell2mat(path_length);
    path_length(path_length == 0) = 1000;
    [~,sort_idx] = sort(path_length);
    path = TR{sort_idx};

    if isempty(path)
      local_idx_neibor = [];
    else
      local_idx_neibor = path(end);
    end
end
