    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
  		...
  		pre_label_5, pre_label_6, pre_label_7, pre_label_8 = MODEL.get_stage_1(pc_coord_cropped, pc_fea_cropped, dof_feat, is_training_pl,bn_decay=bn_decay)						
                ...
                loss_5, recall_5, acc_5, loss_6, recall_6, acc_6, loss_7, recall_7, acc_7, loss_8, recall_8, acc_8 = MODEL.get_stage_1_loss(pre_label_1, label_1, pre_label_2, label_2, pre_label_5, label_5, pre_label_6, label_6,pre_label_7, label_7, pre_label_8, label_8)
...
def eval_one_epoch_stage_1(sess, ops, train_writer):
    is_training = True
    dataset = ['./test_data_2_1/test_data.mat']
...
#        pred_labels_corner_p_val = np.zeros((num_data, NUM_POINT, 2), np.float32)
	...
	input_corner_pair = np.zeros((num_data,NUM_ROI,2),np.float32)
        input_label_1 = np.zeros((num_data,NUM_ROI),np.int32)  
        input_label_2 = np.zeros((num_data,NUM_ROI),np.int32)
        input_label_5 = np.zeros((num_data,NUM_POINT),np.int32)
        input_label_6 = np.zeros((num_data,NUM_POINT),np.int32)
        input_label_7 = np.zeros((num_data,NUM_POINT),np.int32)
        input_label_8 = np.zeros((num_data,NUM_POINT),np.int32)
        ...
        for x in range(num_data):
            ...
            input_label_1[x,:] = np.squeeze(tmp_data['train_all_pair_con_rand'][0,0])
            input_label_2[x,:] = np.squeeze(tmp_data['train_all_pair_not_rand'][0,0])
            input_label_5[x,:] = np.squeeze(tmp_data['open_label'][0,0])
            input_label_6[x,:] = np.squeeze(tmp_data['closed_label'][0,0])
            input_label_7[x,:] = np.squeeze(tmp_data['oli_label'][0,0])
            input_label_8[x,:] = np.squeeze(tmp_data['ocy_label'][0,0])
		...
		batch_corner_pair[cnt,:,:] = tmp_data['train_all_pair_rand'][0, 0]
            ...
            summary, step, _, loss_1_val, recall_1_val, acc_1_val, \
				          ...
				          loss_8_val, recall_8_val, acc_8_val, \
                                 pre_label_1[begin_idx:end_idx,:,:], \
						  ...
						  ops['loss_8'], ops['recall_8'], ops['acc_8'], \
                                                  ops['pre_label_1'],ops['pre_label_2'], \
                                                  ops['pre_label_5'],ops['pre_label_6'], \
                                                  ops['pre_label_7'],ops['pre_label_8']], feed_dict=feed_dict) 
        ...
        total_recall_1 = total_recall_1 * 1.0 / num_batch
        total_acc_1 = total_acc_1 * 1.0 / num_batch
        total_loss_2 = total_loss_2 * 1.0 / num_batch
        total_recall_2 = total_recall_2 * 1.0 / num_batch
	...
	total_loss_8 = total_loss_8 * 1.0 / num_batch
        ...
        sio.savemat('./test_result_2_1/test_pred_'+temp_name, {'input_points_edge_corner': input_points_edge_corner, \
                                                    ...
                                                    'input_label_5': input_label_5, \
	                                            'input_label_6': input_label_6, \
                                                    'input_label_7': input_label_7, \
	                                            'input_label_8': input_label_8, \
                                                    'pre_label_1': pre_label_1, \
                                                    'pre_label_2': pre_label_2, \
                                                    'pre_label_5': pre_label_5, \
                                                    'pre_label_6': pre_label_6, \
